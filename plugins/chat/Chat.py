...
class Chat(Plugin):
    ...
    def __init__(self, users, database, rooms, packet):
        super(Chat, self).__init__(users, database, rooms, packet)
    ...
    def send_emote(self, data, user):
        ...
        self.packet.send_room(["se", user.get_int_id(self.rooms),
                               user.data.id, emote], user.data.room)
